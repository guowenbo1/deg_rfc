...
p <- ggplot(data, aes(x = .data[[group]],y = log10(.data[[index]]), color = .data[[group]])) +
  geom_boxplot() + xlab("") + ylab(paste0("log10(", index, ")")) +
  theme_bw() + scale_color_manual(values=myPalette, name = NULL) + 
  theme(
