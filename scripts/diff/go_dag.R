...
go_enrich_BP <- enricher1(gene_list, pvalueCutoff = 1 , qvalueCutoff = 1, 
                      TERM2GENE = go_BP[,c(3,1)],TERM2NAME = go_BP[,c(3,6)])

file1 = paste(outdir,"DAGplot_BP.pdf", sep="/")
pdf(file1)
plotGOgraph(go_enrich_BP,firstSigNodes = 10,useInfo =  "counts")
dev.off

#使用CC绘制有向无环图
go_CC<- go[which(go$ONTOLOGY=="CC"),]
go_enrich_CC <- enricher2(gene_list, pvalueCutoff = 1 , qvalueCutoff = 1, 
                      TERM2GENE = go_CC[,c(3,1)],TERM2NAME = go_CC[,c(3,6)])

file1 = paste(outdir,"DAGplot_CC.pdf", sep="/")
pdf(file1)
plotGOgraph(go_enrich_CC,firstSigNodes = 10,useInfo =  "counts")
dev.off()

#使用MF绘制有向无环图
go_MF<- go[which(go$ONTOLOGY=="MF"),]
go_enrich_MF <- enricher3(gene_list, pvalueCutoff = 1 , qvalueCutoff = 1, 
                      TERM2GENE = go_MF[,c(3,1)],TERM2NAME = go_MF[,c(3,6)])

file1 = paste(outdir,"DAGplot_MF.pdf", sep="/")
pdf(file1)
plotGOgraph(go_enrich_MF,firstSigNodes = 10,useInfo =  "counts")
dev.off()
