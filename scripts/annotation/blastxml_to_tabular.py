...
if out_fmt == "std":
    ...
    if extra:
        sys.exit("These are not recognised column names: %s" %
                 ",".join(sorted(extra)))
    del extra
    assert set(colnames).issuperset(cols), cols
    if not cols:
        sys.exit("No columns selected!")
    extended = max(colnames.index(c)
                   for c in cols) >= 12  # Do we need any higher columns?
del out_fmt
...
def convert(blastxml_filename, output_handle):
    ...
    try:
        context = ElementTree.iterparse(
            blastxml_filename, events=("start", "end"))
    except Exception:
        ...
        if event == "end" and elem.tag == "Iteration":
            ...
            for hit in elem.findall("Iteration_hits/Hit"):
                ...
                for hsp in hit.findall("Hit_hsps/Hsp"):
                    ...
                    assert len(q_seq) == len(
                        h_seq) == len(m_seq) == int(length)
                    gapopen = str(len(q_seq.replace('-', ' ').split()) - 1 +
                                  len(h_seq.replace('-', ' ').split()) - 1)

                    mismatch = m_seq.count(
                        ' ') + m_seq.count('+') - q_seq.count('-') - h_seq.count('-')
                    ...
                    expected_mismatch = len(q_seq) - sum(1 for q, h in zip(q_seq, h_seq)
                                                         if q == h or q == "-" or h == "-")
                    xx = sum(1 for q, h in zip(q_seq, h_seq)
                             if q == "X" and h == "X")
                    ...
                    expected_identity = sum(
                        1 for q, h in zip(q_seq, h_seq) if q == h)
                    ...
                    values = [qseqid,
                              ...
                              # hsp.findtext("Hsp_evalue") in scientific notation
                              evalue,
                              # hsp.findtext("Hsp_bit-score") rounded
                              bitscore,
                              ]

                    if extended:
                        try:
                            sallseqid = ";".join(name.split(None, 1)[
                                                 0] for name in hit_def.split(" >"))
                            salltitles = "<>".join(name.split(
                                None, 1)[1] for name in hit_def.split(" >"))
                        ...
                        ppos = "%0.2f" % (
                            100 * float(positive) / float(length))
